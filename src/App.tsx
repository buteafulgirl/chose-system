...
import {
  Prize,
  Participant,
  LotterySettings as LotterySettingsType,
  LotteryConfig,
  ParticipantList,
  ParticipantListData,
} from "./types/lottery";
...
function App() {
  ...
  const [prizes, setPrizes] = useState<Prize[]>([
    { id: "1", number: 1, name: "銅獎-日本千石瞬熱石墨烤箱", drawCount: 3 },
    { id: "2", number: 2, name: "銀獎-Apple Watch SE3", drawCount: 2 },
    { id: "3", number: 3, name: "金獎-Dyson吹風機", drawCount: 2 },
  ...
  const [allResults, setAllResults] = useState<
    { prize: Prize; winners: Participant[] }[]
  >([]);

  const [effectiveParticipantsForDraw, setEffectiveParticipantsForDraw] =
  ...
  const completeSettings = () => {
    const totalParticipants = participantLists.reduce(
      (sum, list) => sum + list.participants.length,
      0
    );
  ...
  const startPrizeDraw = (prize: Prize) => {
    ...
    if (prize.participantListId) {
      // 綁定到特定名單
      const targetList = participantLists.find(
        (l) => l.list.id === prize.participantListId
      );
  ...
  const handleWinnerDataSave = useCallback(
    (selectedWinners: Participant[]) => {
        ...
        setParticipantLists((prev) =>
          prev.map((list) => ({
            ...
            participants: list.participants.map((p) => ({
              ...p,
              isSelected:
                selectedWinners.some((w) => w.id === p.id) || p.isSelected,
            })),
          }))
        );
      }
      ...
      if (currentPrize) {
        ...
        const existingResultIndex = allResults.findIndex(
          (result) => result.prize.id === currentPrize.id
        );
  ...
  const exportConfig = () => {
    ...
    const exportFileDefaultName = `lottery-config-${new Date()
      .toISOString()
      .slice(0, 19)
      .replace(/[:-]/g, "")
      .replace("T", "-")}.json`;
  ...
  const validateConfig = (config: unknown): config is LotteryConfig => {
    ...
    return (
      ...
      (obj.participantLists as unknown[]).every(
        (listData: unknown) =>
          listData &&
          ...
          typeof (listData as any).list === "object" &&
          "id" in (listData as any).list &&
          "name" in (listData as any).list &&
          Array.isArray((listData as any).participants) &&
          ((listData as any).participants as unknown[]).every(
            (p: unknown) =>
  ...
  const importConfig = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const importedLists = configData.participantLists.map(
          (listData: ParticipantListData) => ({
            list: listData.list,
  ...
  const downloadExcelTemplate = () => {
    ...
    listSheet["!cols"] = [{ width: 15 }, { width: 20 }];
    ...
    prizeSheet["!cols"] = [{ width: 20 }, { width: 15 }, { width: 15 }];
    ...
    participantSheet["!cols"] = [{ width: 15 }, { width: 20 }];
    ...
    reader.onload = (e) => {
      try {
        ...
        if (workbook.SheetNames.includes("參與名單")) {
          ...
          const listJsonData = XLSX.utils.sheet_to_json(listSheet, {
            header: 1,
          }) as (string | number)[][];
        ...
        if (workbook.SheetNames.includes("獎項設定")) {
          ...
          const prizeJsonData = XLSX.utils.sheet_to_json(prizeSheet, {
            header: 1,
          }) as (string | number)[][];
        ...
        if (workbook.SheetNames.includes("參與者清單")) {
          ...
          const participantJsonData = XLSX.utils.sheet_to_json(
            participantSheet,
            { header: 1 }
          ) as (string | number)[][];
          ...
          for (let i = 1; i < participantJsonData.length; i++) {
            ...
            if (row && row[0] && row[1]) {
              ...
              if (!participantsByListId.has(listId)) {
                ...
                const participants =
                  participantsByListId.get(defaultListId) || [];
                participants.push({
                  id:
                    Date.now().toString() +
                    Math.random().toString(36).substr(2, 9),
        ...
        const totalParticipants = Array.from(
          participantsByListId.values()
        ).reduce((sum, p) => sum + p.length, 0);
        if (importedPrizes.length === 0 && totalParticipants === 0) {
          alert(
            "⚠️ 匯入失敗：未找到有效的獎項或參與者資料！\n請確保 Excel 檔案包含正確的工作表和資料格式。"
          );
          return;
        }

        // 組合名單和參與者
        const importedListsWithParticipants: ParticipantListData[] =
                  ...
                  <div className="border-t pt-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-2">
                          Excel模板包含「獎項設定」和「參與者名單」兩個工作表。支援
                          .xlsx 和 .xls 格式
                        </p>
                      </div>
                    </div>
